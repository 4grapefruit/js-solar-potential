...
<script lang="ts">
  ...
  const icon = 'finance_mode';
  const title = 'Solar Potential Analyse';
  ...
  $: GoogleCharts.load(
    () => {
      ...
      const options = googleCharts.Line.convertOptions({
        title: `Kosten Analyse für ${installationLifeSpan} Jahre`,
        width: 350,
        height: 200,
      });
...
<Expandable
  ...
  {icon}
  {title}
  subtitle="Eigene Werte eingeben"
  secondary
>
  <div class="flex flex-col space-y-4 pt-4">
    ...
    <InputMoney
      ...
      icon="credit_card"
      label="Ø monatliche Stromrechnung"
    ...
    <div class="flex flex-col items-center w-full">
      ...
      >
        Erweiterte Einstellungen {showAdvancedSettings ? 'ausblenden' : 'einblenden'}
        <md-icon slot="icon">
    ...
    {#if showAdvancedSettings}
      <div class="flex flex-col space-y-4" transition:slide={{ duration: 200 }}>
        <InputNumber
          ...
          icon="date_range"
          label="Lebensdauer der Solaranlage"
          suffix="Jahre"
          onChange={updateConfig}
        />
        ...
        <InputRatio
          ...
          icon="trending_down"
          label="Panel Wirkungsgradverlust pro Jahr"
          decrease
          onChange={updateConfig}
        />
        ...
        <InputRatio
          ...
          icon="price_change"
          label="Jährliche Strompreiserhöhung"
          onChange={updateConfig}
        />
      ...
      <div class="w-full secondary-text">
        <Table
          rows={[
            {
              icon: 'wallet',
              name: 'Kosten ohne Solaranlage',
              value: showMoney(totalCostWithoutSolar),
            },
            {
              icon: 'wb_sunny',
              name: 'Kosten mit Solaranlage',
